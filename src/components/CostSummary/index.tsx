...
export const CostSummary = (props: CostSummaryProps) => {
  ...
  const ETC = NumberPrecision.divide(
    NumberPrecision.minus(BAC, BCWP),
    CPI
  ).toFixed(2);
      ...
      <div className="row">
        ...
        <div className="col-6">
          <ul>
            ...
            <li>BCWP = {BCWP}</li>
            <li>
              CPI = BCWP / ACWP = {BCWP} / {ACWP} = {CPI}{" "}
              {isNaN(Number(CPI))
                ? Number(CPI) < 1
                  ? " < 1 => Vượt chi phí"
                  : ">= 1 => Không vượt chi phí"
                : ""}
            </li>
            <li>
              SPI = BCWP / BCWS = {BCWP} / {BCWS} = {SPI}{" "}
              {isNaN(Number(SPI))
                ? Number(SPI) < 1
                  ? " < 1 => Chậm tiến độ"
                  : " >= 1 => Kịp tiến độ"
          ...
          <ul>
            ...
            <li>
              EAC: Chi phí tổng cộng ước tính để hoàn tất = Chi phí thực tế +
              Chi phí ước tính để hoàn tất phần còn lại
            </li>
          </ul>
        </div>
      </div>
